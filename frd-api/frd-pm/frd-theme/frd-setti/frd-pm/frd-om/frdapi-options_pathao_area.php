...
//FRD OPARATION START:-
     if($FR_VC_SESSION == 1 and $FR_VC_POST == 1){
        ...
        if($FRA == 2){ PR("PATHAO ACCESS TOKEN FAILED"); PR($FRM); }
           ...
           //CITY LIST:-
           // Set the request URL
           $request_url = "$fr_pat_base_url/aladdin/api/v1/zones/$f_zone/area-list";
           ...
           curl_close($ch);
            ...
            if(isset($FRR["code"])){
                if($FRR["code"] == 200){
                    ...
                    $FR_OP_HTML .= "<option value=''>Select Area</option>";
                    foreach($FRc_DATA AS $area){
                        extract($area);
                        $FR_OP_HTML .= "<option value='$area_id/$area_name'>$area_name</option>";
                    }
                }else{
                    //ERROR
                    // PR($FRR);
